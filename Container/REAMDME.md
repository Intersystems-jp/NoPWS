...
- webgateway2026.1のイメージ

    `containers.intersystems.com/intersystems/webgateway:2026.1`
...
```
  iris-webgateway:
    ...
    # 　コメントのままで起動した場合、SSL用の設定が記述されていてもSSLは有効化されません。
...
- ISC_CSP_CONF_FILE
    ...
    詳しはドキュメントもご参照ください：**「[Web サーバが Web ゲートウェイを介してルーティングする要求の指定](https://docs.intersystems.com/irisforhealthlatestj/csp/docbook/DocBook.UI.Page.cls?KEY=GCGI_config_overviews#GCGI_config_webserver_invoke)」にある表のApacheの行**
...
- ISC_CSP_INI_FILE
    ...
    http://localhost:9981/csp/bin/systems/module.cxw
...
2. コンテナを開始します。
    ...
    http://localhost:9981/csp/sys/UtilHome.csp
...
- ドキュメントへのリンク切れを修正
- pull例のタグ名を更新
- [CSP.conf](./webgateway/CSP.conf) の修正（SSLセクションの修正） 
- [docker-compose.yml](./docker-compose.yml)の記述変更に伴い例文変更
  - WebGateway用service(iris-webgateway)にcommandを追加
  - iris 用service(nopwsiris)にキー配置用のcommandを追加